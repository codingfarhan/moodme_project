...
def detect(image):
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.putText(image, 'Face', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color=(255, 0, 0), thickness=2)
        ...
        for (ex,ey,ew,eh) in eyes:
            ...
            cv2.putText(roi, 'Eyes', (ex, ey-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,255,0), 2)
            
        # Drawing rectangle around smile
        for (sx,sy,sw,sh) in smile:
            cv2.rectangle(roi, (sx, sy), (sx+sw, sy+sh), (0,0,255), 2)
            cv2.putText(roi, 'Lips', (sx, sy-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 2)
...
def main():
    st.title("Face Detection App For Internship at Moodme :sunglasses: ")
